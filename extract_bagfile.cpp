...
class BasicBehavior {
	...
	BasicBehavior(ros::NodeHandle& nh) :
		...
		rotateDuration(0.f) {
			...
			cmd_velListener = nh.subscribe("cmd_vel", 1, &BasicBehavior::cmd_velListenerCallback, this);
	...
	void cmd_velListenerCallback(const geometry_msgs::Twist::ConstPtr& msg){
		geometry_msgs::Twist vel;
		vel = *msg ;
		ofstream outfile;
		outfile.open("cmds.txt", std::ios_base::app);
		outfile<<vel.linear.x<<" "<<vel.angular.z<<endl ;
		outfile.close();
	...
	protected:
		...
		ros::Subscriber cmd_velListener;
